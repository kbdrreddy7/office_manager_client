<template>
<div>
   <keep-alive exclude="evaluation-form" >
    <router-view
        :tab-mode="tabMode"
...
import MultiEvaluationForm from './MultiEvaluationForm'
...
export default {
  ...
  components:{
      'multi-evaluation-form':MultiEvaluationForm
  },
  data () {
    return {
  ...
  computed:{
        ...mapGetters('auth',['userAccess','user']),
         ...
         currentPage(){
          ...
          propsQuery(){
              return (this.parentQuery || {})['treatmentevaldtl']
          },
          propsValues(){
            return (this.parentValues || {})['treatmentevaldtl']
          },
          propOptions(){
              return (this.parentOptions || {})['treatmentevaldtl']
          },
        ...
        */
        currentPageAccess(){
            ...
            if(this.$route.params.suId)
              key='serviceuser/treatmentevaldtl'
  ...
  methods:{
      ...
      //-------------------- CRUD methods
       async viewObj(id,push=true){
        ...
        addObj(push=true){
          ...
          this.values={
            trevl_classification:"Special Education(AUTISM)",
            trevl_disability:"Autism",
            trevl_serviceuser:"SUA3920200131164619",
            trevl_treatment:"TRA4620200131172536",
            trevl_status:"Created"
          }
    ...
    async setUp(){
          ...
          this.path='treatmentevaldtl'

          if(!this.$route.name.includes('evaluation'))
              return ;
            ...
            // if the case ---route is /:master/:id  and the page is refreshed
              ...
              let {name,query:{mode},params:{suId,evaluationId}}=this.$route
              ...
              if(suId)
                ...
                if(name.endsWith('evaluation_create'))  {
                    this.addObj(false)
                  }   
              else if(name.endsWith('evaluation_details'))  {
                    if(mode==='edit' && this.currentPageAccess.auas_edit)
                      await this.editObj(evaluationId,false)
                      else
                    await this.viewObj(evaluationId,false)
                  }
                  else {
                      await  this.loadTableView() 
                  }
...
//             console.log("in treat watch route ",{to,from})
            ...
            if(!to.name.includes('evaluation'))
              return ;
        

            if(to.name==='default_evaluation' || to.name==='evaluation') {
            this.mode=PAGE_MODES.default   
