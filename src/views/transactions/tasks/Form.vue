...
import {taskService,globalService} from '../../../services'
...
export default {
        ...
        */
        ...
        methods:{
                 async getServiceUserDetails(su_id,obj){

                          let {data} = await globalService.getAll('/serviceuser',
                                {su_id,fields:'su_block,su_panchayat,su_village'})

                                if(data && data[0])
                                {
                                        let suDetails=data[0]
                                        obj.task_block=suDetails.su_block
                                        obj.task_panchayat=suDetails.su_panchayat
                                        obj.task_village=suDetails.su_village
                ...
                validateBeforeSubmit(){
                        ...
                        let newErrors={}       
                        if(this.value.task_status!=='Open' && 
                                this.value.task_targetstatus==='NOT ACHIEVED' &&
                               (!this.value.task_cmmt)
                          )
                           newErrors.task_cmmt="Comment is mandatory"     
                        this.errors={...this.errors,...newErrors}

                       if(this.value.task_status!=='Open'&&
                          this.value.task_checkin_time&&
                          (!this.value.task_checkout_time)
                       )         
               ...
               async  update(key,event) {
                        ...
                        if(key==='task_status'){
                                if(updatedCopy.task_status==='Closed')
                                        newDisabledFields.task_targetstatus=null
                                else {
                                        updatedCopy.task_targetstatus='NOT ACHIEVED'
                                        newDisabledFields.task_targetstatus=true
                                }
                        }     
                        
                        if(key==='task_serviceuser'){
                                await this.getServiceUserDetails(event,updatedCopy)
       ...
       async created(){
               ...
               this.mode=this.pageMode
                ...
                if(this.mode==='create')
                {
                        disabledFields.task_status=true
                        disabledFields.task_targetstatus=true
                         
                
                        // if su_id exists ..but not task_block
                        if(this.value.task_serviceuser && (!this.value.task_block))
                        {
                                let obj={}
                                await this.getServiceUserDetails(this.value.task_serviceuser,obj)
        ...
        watch:{
              ...
              async  pageMode(newVal){
                      // this is to handle when page is refreshed
                        ...
                        if(newVal==='create')
                        {
                                this.disabledFields.task_status=true
                                this.disabledFields.task_targetstatus=true
